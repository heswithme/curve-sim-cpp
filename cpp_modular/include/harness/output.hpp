...
inline json::object approx_pool_state_json(const RuntimePoolResult& r) {
    json::object o;
    o["balances"] = json::array{to_str_1e18(r.balances[0]), to_str_1e18(r.balances[1])};
    o["xp"] = json::array{
        to_str_1e18(r.balances[0]),
        to_str_1e18(r.balances[1] * r.price_scale)
    };
    o["D"] = to_str_1e18(r.D);
...
inline json::object pool_state_json(const RuntimePoolResult& r) {
    return r.final_state_json.empty() ? approx_pool_state_json(r) : r.final_state_json;
}

inline json::object compute_apy_metrics(const RuntimePoolResult& r) {
    json::object apy;

    constexpr double SEC_PER_YEAR = 365.0 * 86400.0;
    const double duration_s = r.duration_s();

    if (duration_s <= 0.0 || r.tvl_start <= 0.0) {
        apy["apy"] = -1.0;
        apy["apy_net"] = -1.0;
        apy["apy_xcp"] = -1.0;
        apy["apy_xcp_net"] = -1.0;
    ...
    const double donation_apy = r.donation_apy;
    const double donation_freq_s = r.donation_frequency;
    ...
    const double vp_end = r.virtual_price;
    const double apy_vp = (vp_end > 0.0) ? std::pow(vp_end, exponent) - 1.0 : -1.0;
    const double apy_net = net_apy_from_growth(vp_end);
    const double xcp_end = (r.xcp_profit + 1.0) / 2.0;
    const double apy_xcp = (xcp_end > 0.0) ? std::pow(xcp_end, exponent) - 1.0 : -1.0;
    const double apy_xcp_net = net_apy_from_growth(xcp_end);
...
inline void append_core_metrics(
    json::object& summary,
    const RuntimePoolResult& r,
    size_t n_events,
    ...
    summary["avg_pool_fee"] = m.avg_pool_fee();
    summary["tw_avg_pool_fee"] = tw_summary.tw_avg_pool_fee;
    summary["arb_pnl_coin0"] = m.arb_pnl_coin0;

    const double gross_lvr = m.lp_fee_coin0 + m.arb_pnl_coin0;
    summary["fee_capture_rate"] = (gross_lvr > 0.0) ? (m.lp_fee_coin0 / gross_lvr) : -1.0;
    ...
    summary["donation_amounts_total"] = json::array{
        m.donation_amounts_total[0],
        m.donation_amounts_total[1]
    };
    ...
    summary["cowswap_notional_coin0"] = m.cowswap_notional_coin0;
    summary["cowswap_lp_fee_coin0"] = m.cowswap_lp_fee_coin0;
    summary["pool_exec_ms"] = r.elapsed_ms;
}

inline void append_slippage_metrics(json::object& summary, const RuntimePoolResult& r) {
...
inline void append_tvl_metrics(json::object& summary, const RuntimePoolResult& r) {
    summary["t_start"] = r.t_start;
    summary["t_end"] = r.t_end;
    summary["duration_s"] = r.duration_s();
    summary["tvl_coin0_start"] = r.tvl_start;

    const double tvl_end = r.balances[0] + r.balances[1] * r.price_scale;
    summary["tvl_coin0_end"] = tvl_end;
    summary["tvl_growth"] = (r.tvl_start > 0.0) ? (tvl_end / r.tvl_start) : -1.0;
}

inline void append_apy_metrics(json::object& summary, const RuntimePoolResult& r) {
...
inline void append_end_state_metrics(json::object& summary, const RuntimePoolResult& r) {
    summary["vp"] = r.virtual_price;
    summary["vp_boosted"] = r.vp_boosted;
    summary["vpminusone"] = r.virtual_price - 1.0;
}

inline json::object metrics_to_summary(const RuntimePoolResult& r, size_t n_events) {
    json::object summary;
...
inline json::object build_output_json(
    const std::vector<RuntimePoolResult>& results,
    size_t n_events,
...
inline bool write_results_json(
